...
The dataset has only 3 notebook written in Python and only one notebook has implemented a prediction using SARIMA model.   
...
From this first plot, we can first notice that this time series has for the most part of it a **positive linear trend**, a multiplicative seasonal patterns, and some irregular patterns. This plot suggests to use what we call a SARIMA model to do our forecasting.
...
From the ACF, we can see that the values are outside the confidence interval represented by the blue lines. It means that they are all different from zero and so significant, i.e they depend on time. 
...
**P=2** #AR because some significant coefficients in the PACF remain at order 12 & at order 24. We will use P=1 as we got an error with P=2 using method = 'ML'
...
mod1
```
We have here an Akaike Information Criteria (AIC) of -685. AIC is widely used measure of a statistical model. It basically quantifies 1) the goodness of fit, and 2) the simplicity/parsimony, of the model into a single statistic which will be compared with other AIC of different model (the lower the better). 
...
However, we can observe that some parts of the model do not fit well data based on the real data for instance, at year 2009. 
...
mod1
```
We can notice that our AIC is slightly better than before (which was -685). That means that our model is fitting better with our data.  
