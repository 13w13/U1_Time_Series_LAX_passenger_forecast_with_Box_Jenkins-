...
mod1 <- arima(ltraf, c(2, 1, 1),seasonal = list(order = c(4, 0, 2), period = 6), method='ML')
mod1
...
#arima
mod1 <- arima(ltraf, c(2, 1, 1),seasonal = list(order = c(1, 0, 1), period = 12), method='ML')
mod1
```
...
```
```{r}
mod1 <- arima(ts(ltraf, frequency=1), c(2, 1, 0),seasonal = list(order = c(1, 0, 1), period = 12), method='ML')
mod1
```
...
```{r}
mod1 <- arima(ts(ltraf, frequency=1), c(2, 1, 0),seasonal = list(order = c(0, 1, 1), period = 12), method='ML')
mod1
```
```{r}
tstat <- mod1$coef/sqrt(diag(mod1$var.coef))
