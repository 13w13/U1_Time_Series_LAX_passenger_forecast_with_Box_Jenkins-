...
We download the data and upload it in our working github repo for more convinency. 
The dataset has on ly 3 notebook written in Python and only one notebook has implemneted a prediction using SARIMA model.   

Thus, it seemed relevant to us to write this script in R by applying the Box Jenkins method in order to make predictions on this dataset using the methods seen during the Time Series by Anne Valhems. 
...
**Outline**  : 
After loading the data, we are going to analyze the data, visualize our data to understand it better. (Chapter 1) Then, we will focus on the sationary analysis by including graphds and tables to justify our analysis. We will transforme the data in order to reach stationarity.(Chapter 2)
The stationarity reached, we will then apply Box-Jenkins methodology to build an ARMA process on the modified data and select the best model (Chapter 3). 
...
Lax_init %>% str
```
As mentioned below, we load the data from our github repo. 
...
We create a new Lax dataset with only the data we are interessted in for the prediction. 
```{r}
...
frequency(traf)
```
Our time series data strats at date 2006 january and ends at date 2019 mars. It's frequency is monthly. 
...
Let's deepn our visual analysis with the decompisation of the time serie into trend, seasonality and random component. 
...
plot(x)
```
We find the same conlusion from visual observation of time series, i.e a mainly positive linar trend over time, a seasonal patterns with some irregular patterns in the random component (variation with Time and this variance was not captured by Sesonality nor the trend)


### 1.6 Visualization of the frequency
Our final plot for this description part will be the visualisation of the seasonality within our time series with monthplot function. 
```{r}
monthplot(traf)
```
Monthplot function plots the average value obtain at each month (season). 
We can notice tha in average, passenger trafic tends to decrease between january and fabruary then increases between march and august to decrease in the month of september and then remain stable between october and december. 

This concludes our description part. Let's move to stationarity analysis and transformation. 


## 2. Stationarity ? 

A stationary processes have no trend in mean, and no trend in variance and so auto-covariances do not depend on time, but only on the difference of time. 
Let's check the auto and partial autocorrelation function (ACF and PACF) of our raw data in order to confirm  what we have observed from our first plot. 

```{r}
acf(ts(traf, frequency=1))
pacf(ts(traf, frequency=1))
```
Blue : confidence internval. If the values were insdide the interval, it would mean that there anough close to zeros to be non significant. 
...
This trend seems to be deterministic, linear and non-stochastic (but we will use a sotchastic modelisation for the trend using integration within ARIMA)
...
```
### 2.2 Firt order differnce : remove the trend
```{r}
...
D is equal to the seaonality difference
...
mod1
```
We have hare an Akaike Information Critera (AIC) of -685. AIC is widely used measure of a statistical model. It basically quantifies 1) the goodness of fit, and 2) the simplicity/parsimony, of the model into a single statistic which will be compared with other AIC of different model (the lower the better). 
...
We should now pass to test the significance of our coeficient. 
...
ar1, ar2, sar1, ma1 are not sigificants in this first model (pvalue>5%)
We will remove each by each the parameters not significants and re-estimate the model without them. 
...
mod1
```
We can notice that our AIC is slicly better than before (-685). That means that our model is 
...
- Wether we decide it is a non plossible value and change this value with what happens before
- tranform them in a dummy. 
...
qqline(res1)
```
Here, from the qq plot we can notice that our model data does not follow qqline which follows normal distriubtion. 
...
```
We have an issue : 
the first order coef of the ACF and the 22 are sigficiants. So there is a link between the value of the residuals at date t, and the square value of the residuals at date 1 and date 22 

=> It may be due to the outlier we identified
=> Or maybe we shoud add a model more complex 
...
### 3.5 - Builing a SARIMAX model : dummy variables
...
#### Run our model
For notice, in order to incoporate multiple dummies variable, we have to create a cbind matrix into xreg parameters. 
```{r}
...
prop
```
Here, the proprotion is larger than 80%, then the fit is considered good.
...
Run our diffent model on train.ltraf
```{r}
train.mod1 <- Arima(train.ltraf, c(2, 1, 1),seasonal = list(order = c(1, 1, 1), period = 12), method='ML')
train.mod2 <- Arima(train.ltraf, c(0, 1, 1),seasonal = list(order = c(1, 1, 1), period = 12), method='ML')
train.mod3 <- Arima(train.ltraf, c(0, 1, 1),seasonal = list(order = c(0, 1, 1), period = 12), method='ML')
```


```{r}
train.mod4 <- Arima(train.ltraf, c(0, 1, 1),seasonal = list(order = c(0, 1, 1), period = 12), 
              method='ML',xreg=cbind(unlist(Lax_data$dum_1), 
                                     unlist(Lax_data$dum_2),
                                     unlist(Lax_data$dum_3),
                                     unlist(Lax_data$dum_4)))
...
```{r}
plot(train.ltraf)
lines(fitted(train.mod4), col='red')
```

```{r}
tsforecasts4 <- forecast(train.mod4)
a4<-accuracy(tsforecasts4,test.ltraf)
a4
```
